...
| Admin Panel | ✅ | `NewAdminModal.tsx` — same two-column shell as settings, all tabs as left-rail nav items; light-mode text inheritance fixed (Phase 22). A11y Pass 1: focus trap added, `aria-labelledby` wired. Phase 53: same × / section-title header-row alignment fix as settings; keeps its inline "● unsaved" badge and unsaved-changes confirm on close. Phase 53 Fix 4: now REPLACES the settings modal instead of rendering inside its overlay — the settings panel no longer shows behind it, and closing admin returns to the app. Also fixed a double-Escape bug that let Escape discard admin's unsaved changes even when the confirm was cancelled. **Phase 67:** All 10 tab content sections (Configurations, Supported Models, Application Variables, OpenAI Endpoints, Feature Flags, Feature Data, Ops, Embeddings, Integrations, Critical Errors) restyled to new-UI design tokens via `[data-new-ui-admin-content="true"]` CSS scope in `conversation-view.css`. gradient-header overridden to flat surface. All inputs, tables, cards, borders use design tokens. 🚧 Portal modals (Manage Members, CSV Upload) are outside the scope — readable but not pixel-matched. |
