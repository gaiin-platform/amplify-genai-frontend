...
| `shareHistory.ts` | localStorage-backed, user-scoped record of outgoing shares. Exports `loadShareHistory`, `recordShare`, `getSharesForSource`. Call `recordShare` only after a successful `shareItems()` resolve. No React imports |
