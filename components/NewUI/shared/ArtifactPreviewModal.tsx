      ...
      <section role="dialog" aria-modal="true" aria-labelledby="artifact-preview-title" className="flex flex-col w-full max-w-5xl h-[min(760px,90dvh)] rounded-[14px] border shadow-2xl" style={{ backgroundColor: 'var(--bg-app)', borderColor: 'var(--border-subtle)' }}>
        <header className="flex items-center gap-3 px-5 py-4 border-b flex-shrink-0" style={{ borderColor: 'var(--border-subtle)' }}>
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[12px] mt-1" style={{ color: 'var(--text-muted)' }}>{item.nuiType} · v{item.artifact.version}{item.source.conversation ? ` · ${item.source.conversation.name}` : ''}</p>
          </div>
        ...
        <footer className="flex items-center justify-between px-5 py-3 border-t flex-shrink-0" style={{ borderColor: 'var(--border-subtle)' }}>
          <span className="text-[12px] truncate" style={{ color: 'var(--text-muted)' }}>{item.source.conversation ? `Generated in ${item.source.conversation.name}` : 'Source conversation unavailable'}</span>
          <button type="button" disabled={!item.source.conversation || !onOpenChat} onClick={onOpenChat} className="inline-flex items-center gap-1.5 h-8 px-3 rounded-[7px] text-[12px] font-medium disabled:opacity-40" style={{ color: 'var(--accent-fg)', backgroundColor: 'var(--accent)' }}><IconArrowUpRight size={14} /> Open in chat</button>
        </footer>
      </section>
    </div>,
    document.body,
