import { FC, useContext, useRef, useState, useEffect, useCallback } from 'react';
...
export const IntegrationsPanel: FC<Props> = ({ onSave, onCancel, isDirty }) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('saveIntegrationsChanges', handleSaveEvent);

    return () => {
      window.removeEventListener('saveIntegrationsChanges', handleSaveEvent);
    };
  }, [handleSave]);
