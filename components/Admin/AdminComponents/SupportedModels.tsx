...
export const SupportedModelsTab: FC<Props> = ({availableModels, setAvailableModels, ampGroups, 
    ...
    const modelNumberInputs = (key: string, value: number | null, step: number, parseInteger: boolean, description: string) => 
        ...
        <input type="number" id={`${key}Input`} title={description}
                className="text-center w-[100px] bg-brand-light-input dark:bg-brand-dark-input bg-gray-200"
                        ...
                        <div className="flex-grow">
                            ...
                            <div className="flex flex-row"> 
                                ...
                                { ModelProviders.map((p:string) => 
                                    <button key={p}
                                    id={`provider${p}`}
                                    className={`w-[182.5px] h-[39px] rounded-r border border-neutral-500 px-4 py-1 bg-brand-light-input dark:bg-brand-dark-input bg-gray-300 dark:text-neutral-100 text-neutral-900 shadow focus:outline-none dark:border-neutral-800 dark:border-opacity-50 
                        ...
                        {Object.values(availableModels)
                            ...
                            <tr key={availModel.id}  className={`text-xs ${hoveredModelIcons === availModel.id ? 'hover:bg-gray-200 dark:hover:bg-[#40414f]' : ''}`}
                                ...
                                <td className="border border-neutral-500 break-words ">
                                    <div className="flex justify-center p-2 bg-brand-light-input dark:bg-brand-dark-input">
                                        <select 
                                        ...
                                        >
                                            {ModelProviders.map((provider: string) => (
                                                <option key={provider} value={provider} className="bg-white bg-brand-light-input dark:bg-brand-dark-input text-neutral-900 dark:text-neutral-100">
                                                    {provider}
                                                </option>
                                            ))}
                                        </select>
...
const ModelDefaultSelect: FC<SelectProps> = ({models, defaultModels, selectedKey, description, onSelect, label}) => {
    ...
    return (
        <div className="flex flex-col gap-2 text-center text-[14px] " title={description}>
            ...
            <select id="modelSelect" className={`mb-2 text-center rounded-lg border py-2 text-neutral-900 shadow focus:outline-none bg-brand-light-input dark:bg-brand-dark-input dark:text-neutral-100  custom-shadow
